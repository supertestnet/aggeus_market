...
4. the Capital Efficiency Problem -- to create a contract worth N sats, a user must deposit that amount into a bitcoin address, and for another user to "enter" that contract, the coordinator must put an *additional* N sats into a bitcoin address similar to the first one. As a result, every active contract has a capital cost of 2x the amount it should, though that money ends up going to the rightful owners when the contract resolves, so it is only a temporary capital cost; also, since the coordinator always provides the extra funding, he or she can cover the cost of capital via service fees
