...
4. the Capital Efficiency Problem -- to create a contract worth N sats, a user must deposit that amount into a bitcoin address, and for User B to "enter" that contract, the coordinator must put an *additional* N sats into a bitcoin address similar to the first one. As a result, every contract theoretically has a capital cost of 2x the amount it should, though that money ends up going to the rightful owners when the contract resolves
